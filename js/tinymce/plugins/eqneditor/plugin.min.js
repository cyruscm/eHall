...
tinymce.PluginManager.add("eqneditor",function(editor, url) {
	...
	sl.add('/js/eq_config.js');
	sl.add('js/eq_editor.js');
	...
	function showDialog() {
		...
		win = editor.windowManager.open({
			...
			buttons : [
					{
						type:'container',
						html:'<span style="font-size:11px;"><a href="http://www.codecogs.com" target="_blank" style="font-size:11px"><img src="'+http+'latex.codecogs.com/images/poweredbycc.gif" alt="Powered by CodeCogs" style="vertical-align:-7px; border:none"/></a> It is highly suggested that you use a external LaTeX editor if you heavily use this feature. '
					},
					{type: "spacer", flex: 1},
					{
						text: 'Ok',
						...
						minWidth: 50,
						onclick: function() {	
              var a = EqEditor.getTextArea().exportEquation('html');
              a = a.replace('" title=', '}" title=');
							editor.execCommand('mceInsertContent', false, a.replace("png.latex?", "svg.latex?{\\color{White}"));
